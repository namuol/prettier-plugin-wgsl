...
export function printWgsl({text, statements}: ParsedWgsl) {
  ...
  function printWgslNode(node: Node): Doc {
    switch (node.astNodeType) {
      ...
      case 'typecastExpr':
        // typecastExpr nodes are not currently generated by the parser Type
        // conversions like f32(value) are handled by createExpr instead
        return text.slice(node.start, node.start + node.length);
      case 'override':
        return printOverride(node as Override);
      case 'staticAssert':
        // staticAssert nodes are not currently generated by the parser Static
        // assertions are not yet supported in WGSL
        return text.slice(node.start, node.start + node.length);
      case 'constExpr':
        // constExpr nodes are not currently generated by the parser Constant
        // expressions are evaluated and become literals instead
        return text.slice(node.start, node.start + node.length);
      case 'defaultSelector':
        // defaultSelector nodes are not currently generated by the parser
        // Default cases in switch statements are handled by the 'default' case
        // instead
        return text.slice(node.start, node.start + node.length);
      default:
        return text.slice(node.start, node.start + node.length);
  ...
  function printLoop(node: Loop): Doc {
    ...
    if (node.body.length > 0) {
      // Filter out continuing statements from body since they have special
      // handling
      const bodyStatements = node.body.filter(
        (stmt) => stmt.astNodeType !== 'continuing',
      );
      if (bodyStatements.length > 0) {
        bodyParts.push(join(hardline, bodyStatements.map(printStatement)));
      }
    }
